...
Shakespear:
step 5000: train loss 0.6180, val loss 1.7066
iter 5000: loss 0.8177, time 3161.66ms, mfu 15.52%

PAIPASSWORD=123 CUDA_VISIBLE_DEVICES=1 python train_perforatedai.py config/train_shakespeare_char.py --compile=False
train_perforated with smaller shakespear
iter 6740: loss 1.2220, time 17.19ms, mfu 11.81%
step 6750: train loss 1.0760, val loss 1.4545
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process and iter_num > 0:
        ...
        GPA.pai_tracker.add_extra_score(losses["train"], "train")
        model, restructured, trainingComplete = GPA.pai_tracker.add_validation_score(
            losses["val"], model
        )  # .module if its a dataParallel
        model.to(device)
        if trainingComplete:
            break
        elif (
            restructured
        ):  # if it does get restructured, reset the optimizer with the same block of code you use above.
            optimizer = model.configure_optimizers(
                weight_decay, learning_rate, (beta1, beta2), device_type
            )
            iter_num = 0
